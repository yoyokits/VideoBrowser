namespace YoutubeDlGui.Helpers
{
    ...
    public class FFMpeg
    {
                                                 ...
                                                 Action<int> reportProgress)
        {
            ...
            var arguments = string.Format(Commands.Combine, video, audio, output);
            var lines = new StringBuilder();
            ...
            var p = ProcessHelper.StartProcess(FFMpegPath, arguments,
                ...
                delegate (Process process, string line)
                {
                    ...
                    {
                        var lineStart = "Duration: ".Length;
                        var length = "00:00:00.00".Length;
                        var time = line.Substring(lineStart, length);
                    ...
                    {
                        var lineStart = line.IndexOf("time=") + 5;
                        var length = "00:00:00.00".Length;

                        var time = line.Substring(lineStart, length);

                        var currentMilli = TimeSpan.Parse(time).TotalMilliseconds;
                        var percentage = (currentMilli / milliseconds) * 100;
                                              ...
                                              Action<int, object> reportProgress)
        {
            ...
            var p = ProcessHelper.StartProcess(FFMpegPath, arguments,
                ...
                delegate (Process process, string line)
                {
                    ...
                    if (line.StartsWith("Duration: "))
                    {
                        var start = "Duration: ".Length;
                        var length = "00:00:00.00".Length;
                        var time = line.Substring(start, length);
                        milliseconds = TimeSpan.Parse(time).TotalMilliseconds;
                    }
                    else if (line == "Press [q] to stop, [?] for help")
                    {
                                                     ...
                                                     CancellationToken ct)
        {
            ...
            var canceled = false;
            var started = false;
            var milliseconds = cropEnd.TotalMilliseconds;
            var lines = new StringBuilder();
                                              ...
                                              CancellationToken ct)
        {
            ...
            var args = new string[]
            {
                $"{start.Hours:00}:{start.Minutes:00}:{start.Seconds:00}.{start.Milliseconds:000}",
                input,
                $"{length.Hours:00}:{length.Minutes:00}:{length.Seconds:00}.{length.Milliseconds:000}",
                output
            };
            var canceled = false;
            var started = false;
            ...
            p = ProcessHelper.StartProcess(FFMpegPath, arguments,
                ...
                delegate (Process process, string line)
                {
                    ...
                    {
                        var lineStart = line.IndexOf("time=") + 5;
                        var lineLength = "00:00:00.00".Length;

                        var time = line.Substring(lineStart, lineLength);
        ...
        private static FFMpegResult<string> GetVersion()
        {
            ...
            var regex = new Regex("^ffmpeg version (.*) Copyright.*$", RegexOptions.Compiled);
            var lines = new StringBuilder();
